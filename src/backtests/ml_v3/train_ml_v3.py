...
class MLTrainerV3:
    ...
    def __init__(self, config: TradingConfig):
        ...
        self.labeler = TripleBarrierLabeling(
            profit_atr_mult=1.5,     # 50% ATR profit target
            stoploss_atr_mult=1.0,   # 50% ATR stop loss (symmetric RR 1.0)
            max_holding_bars=40,     # 5 hours on M15 (allow time to develop)
        )
    ...
    def select_features(self, df: pl.DataFrame) -> List[str]:
        ...
        exclude_cols = {
            ...
            # ---> POISONOUS ABSOLUTE PRICES (Must be excluded!) <---
            "swing_high", "swing_low",            # <-- MISSING FROM YOUR PREVIOUS SCRIPT
            "fvg_mid", "ob",                      # <-- MISSING FROM YOUR PREVIOUS SCRIPT
            ...
            "h1_close", "h1_ema_20", "ema_50", "ema_200", "vwap", "h1_ema_50", "h1_ema_200", 
            "h1_high", "h1_low", "h1_close", "h1_open",
            "struct_high", "struct_low", "struct_mean"
        }
    ...
    def prepare_data_splits(
        ...
        """
        Split data strictly chronologically: Train -> Val -> Test.
        Never shuffle time-series data before splitting!
        """
        print(f"\n Splitting train/val/test (Chronological)...")
        ...
        n_total = len(df)
        val_idx = int(n_total * (1.0 - val_size - test_size))
        test_idx = int(n_total * (1.0 - test_size))

        # Strict chronological split
        df_train = df.head(val_idx)
        df_val = df.slice(val_idx, test_idx - val_idx)
        df_test = df.tail(n_total - test_idx)

        print(f"   Train: {len(df_train):,} samples (Used to fit)")
        print(f"   Val:   {len(df_val):,} samples (Used for Optuna)")
        print(f"   Test:  {len(df_test):,} samples (BLIND - Used for final metrics)")
    ...
    def train_xgboost(
        ...
        weight_buy = n_total / (2 * (n_total - n_sell)) if (n_total - n_sell) > 0 else 1.0
        ...
        # Combine Train + Val for final fit (optional, but good practice if early stopping isn't used)
        X_combined = np.vstack((X_train, X_val))
        y_combined = np.concatenate((y_train, y_val))
        sw_combined = np.concatenate((sample_weights, np.ones(len(y_val)))) # simplified weights
        ...
        # Evaluate strictly on BLIND test set
        print(f"\n Model Performance (BLIND TEST):")
        y_train_pred = model.predict(X_combined)
        y_test_pred = model.predict(X_test)

        train_acc = (y_train_pred == y_combined).mean()
        test_acc = (y_test_pred == y_test).mean()

        print(f"   Train+Val Accuracy: {train_acc:.4f}")
    ...
    def _optimize_hyperparameters(
        self, X_train, y_train, X_val, y_val, sample_weights
    ) -> Dict:
        def objective(trial):
            # HIGHLY CONSTRAINED SEARCH SPACE
            params = {
                'max_depth': trial.suggest_int('max_depth', 2, 4), # Prevents memorization
                'learning_rate': trial.suggest_float('learning_rate', 0.005, 0.05, log=True),
                'n_estimators': trial.suggest_int('n_estimators', 50, 200),
                'min_child_weight': trial.suggest_int('min_child_weight', 20, 150), # Require robust leaf nodes
                'gamma': trial.suggest_float('gamma', 0.0, 1.0),
                'subsample': trial.suggest_float('subsample', 0.5, 0.8),
                'colsample_bytree': trial.suggest_float('colsample_bytree', 0.5, 0.8),
                'reg_alpha': trial.suggest_float('reg_alpha', 0.5, 5.0), # Heavy L1
                'reg_lambda': trial.suggest_float('reg_lambda', 1.0, 10.0), # Heavy L2
            }

            model = xgb.XGBClassifier(
                objective='binary:logistic', random_state=42, n_jobs=1, **params
            )

            model.fit(X_train, y_train, sample_weight=sample_weights, verbose=False)
            
            # NO LEAKAGE: Optuna evaluates on VAL set, not TEST set
    ...
    def run_full_pipeline(self):
        ...
        # 1. Fetch data
        df_m15 = self.fetch_training_data(n_bars=50000)
        df_h1 = self.fetch_h1_data(n_bars=2000)
        ...
        # NOTE: Make sure you copied 'prepare_data_splits' from my previous message
        # into this class so this call works!
        df_train_raw, df_val, df_test = self.prepare_data_splits(df, val_size=0.15, test_size=0.15)
        ...
        # 7. Train model
        model = self.train_xgboost(df_train, df_val, df_test, feature_cols, optimize_hyperparams=True)
