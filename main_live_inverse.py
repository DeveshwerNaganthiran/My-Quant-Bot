...
class TradingBot:
    ...
    def __init__(
        ...
        self._always_invert = True          # ENABLE PERMANENT INVERSION
    ...
    async def _trading_iteration(self):
        ...
        # ALWAYS INVERT
        ml_prediction = self._invert_ml_prediction(raw_ml_prediction)
        smc_signal = self._invert_smc_signal(raw_smc_signal) if raw_smc_signal else None
        mode_tag = "[ALWAYS INVERTED]"
        ...
        if is_inverted:
            can_trade_pullback = True
            pb_reason = "Pullback check bypassed (Momentum Mode)"
            
            # Looser peak prevention for momentum (allows trading into strong trends)
            rsi_val = df["rsi"].drop_nulls().tail(1).item() if "rsi" in df.columns else 50
            if final_signal.signal_type == "BUY" and rsi_val > 82:
                can_trade_pullback = False
                pb_reason = f"BUY blocked: Extreme Parabolic Peak (RSI: {rsi_val:.1f})"
            elif final_signal.signal_type == "SELL" and rsi_val < 18:
                can_trade_pullback = False
        ...
        target_direction = final_signal.signal_type
        is_inverted = getattr(self, '_always_invert', False)
    ...
    async def _verify_and_execute_delayed(self, original_signal, position_result, original_regime_state):
        ...
        try:
            ...
            # ALWAYS INVERT (5s check)
            if getattr(self, '_always_invert', True):
